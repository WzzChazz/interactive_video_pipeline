...
publish_e16.py
==============
重新发布 E16（陷阱中的真相，第4集）— 被标为"不适宜公开"

用法：
    PYTHONPATH=. .venv/bin/python publish_e16.py
"""
...
def main():
    ...
    try:
        url = publish_to_douyin(
            video_path=video_path_16, 
            title=title_cn, 
            caption=final_caption,
            branch_a_teaser=branch_a,
            branch_b_teaser=branch_b
        )
        logger.success(f"✅ E16 发布成功！URL: {url}")
        
        # Update DB
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute(
            "UPDATE episodes SET douyin_video_url=?, status='PUBLISHED' WHERE episode_number=16",
            (url,)
        )
        conn.commit()
        conn.close()
